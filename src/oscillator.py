...
def rk4_solver(m, k, c, omega, F0, x0, v0, dt, tmax):
    ...
    while t < tmax:
        ...
        dx2, dv2 = rhs(
            t + 0.5 * dt, x + 0.5 * dt * dx1, v + 0.5 * dt * dv1, m, k, c, omega, F0
        )
        dx3, dv3 = rhs(
            t + 0.5 * dt, x + 0.5 * dt * dx2, v + 0.5 * dt * dv2, m, k, c, omega, F0
        )
...
def scipy_ivp(m, k, c, omega, F0, x0, v0, dt, tmax):
    ...
    sol = solve_ivp(
        f, (0.0, tmax), [x0, v0], method="RK45", dense_output=True, max_step=dt
    )  # using dense output to get solution at specific times
...
def amp_phase(m, k, c, omega, F0):
    """Amplitude and phase of the steady state solution"""
    omega0 = np.sqrt(k / m)
    beta = c / (2 * m)
...
def steady_amp(t, x, fraction=0.5):
    """Estimate the amplitude of the steady state solution"""
...
def analytic_oscillator(m, k, c, omega, F0, x0, v0):
    """Analytic solution of the driven damped oscillator"""
    omega0 = np.sqrt(k / m)
    beta = c / (2 * m)
    ...
    if beta < omega0:  # underdamped
        ...
        def x(t):
            return np.exp(-beta * t) * (
                A1 * np.cos(omega_d * t) + A2 * np.sin(omega_d * t)
            ) + xp(t)

        def x_dot(t):
            return np.exp(-beta * t) * (
                -beta * (A1 * np.cos(omega_d * t) + A2 * np.sin(omega_d * t))
                + A2 * omega_d * np.cos(omega_d * t)
                - A1 * omega_d * np.sin(omega_d * t)
            ) - X * omega * np.sin(omega * t - phi)
        ...
        def x_dot(t):
            return (B2 - beta * (B1 + B2 * t)) * np.exp(-beta * t) - X * omega * np.sin(
                omega * t - phi
            )
        ...
        regime = "overdamped"
    return x, x_dot, regime


def analytic_energy(m, k, c, omega, F0, x0, v0):
    """Analytic energy of the driven damped oscillator"""
    x, x_dot, regime = analytic_oscillator(m, k, c, omega, F0, x0, v0)

    def E(t):
        return 0.5 * m * x_dot(t) ** 2 + 0.5 * k * x(t) ** 2
